...
class SpeechSynthesizer: UIViewController {
    ...
    @IBOutlet weak var selectSpeech: UISegmentedControl!
    ...
    @IBAction func togglePlay(_ sender: Any?) {
        ...
        let isAWS = self.selectSpeech.selectedSegmentIndex == 1
        if self.isPlaying {
            self.pauseSpeech(isAWS: isAWS)
        }
        else {
            if text == self.text {
                self.continueSpeech(isAWS: isAWS)
            }
            else {
                if isAWS {
                    self.setupAWSPolly(text: text)
                }
                else {
                    self.setupSpeechUtterance(text: text)
                }
            }
...
//        self.arabicSpeechUtterance?.postUtteranceDelay = 0.1
    ...
    func setupAWSPolly(text: String) {
        let input = AWSPollySynthesizeSpeechURLBuilderRequest()
        
        // AWS 只支援簡中，繁中會掉字
        input.text = self.simplify(text)
        
        input.outputFormat = AWSPollyOutputFormat.mp3
        
        input.voiceId = AWSPollyVoiceId.zhiyu
    ...
    @IBAction func changeSpeech(_ sender: Any) {
        self.stopSpeech()
    }
    
    func continueSpeech(isAWS: Bool) {
        if isAWS {
            self.audioPlayer.play()
        }
        else {
...
extension SpeechSynthesizer: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if self.isPlaying {
            self.pauseSpeech(isAWS: self.selectSpeech.selectedSegmentIndex == 1)
        }
    }
...
extension SpeechSynthesizer {
    ...
    func simplify(_ ch: String) -> String {
        return String(ch.map { c in
            if let i = SpeechSynthesizer.big5.index(of: c) {
                return SpeechSynthesizer.gb[i]
            } else {
                return c
            }
        })
    }
    func traditionalize(_ ch: String) -> String {
        return String(ch.map { c in
            if let i = SpeechSynthesizer.gb.index(of: c) {
                return SpeechSynthesizer.big5[i]
            } else {
                return c
            }
        })
